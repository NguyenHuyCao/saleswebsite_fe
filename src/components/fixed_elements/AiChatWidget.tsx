...
export default function AiChatWidget() {
  ...
  // Scroll to bottom — instant while streaming to avoid lag, smooth otherwise
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const historyPromise = fetch(`${BASE_URL}/api/v1/ai/sessions/${sessionId.current}/messages`, {
      credentials: "include",
    })
      ...
      .then((raw) => {
        ...
        if (arr.length > 0) {
          ...
          const maxHistoryTs = Math.max(0, ...restored.filter((m) => m.role === "assistant").map((m) => m.ts));
          if (maxHistoryTs > lastReceivedDbTsRef.current) lastReceivedDbTsRef.current = maxHistoryTs;
          setMessages([WELCOME, ...restored]);
        }
      })
      .catch(() => {});

    const pingPromise = fetch(`${BASE_URL}/api/v1/ai/ping-ai`, { credentials: "include" })
      .then((r) => r.json())
      .then((data) => {
   ...
   * Adapts speed so any response finishes in ~1.5-3s regardless of length.
   */
  const runTypewriter = (content: string) => {
    if (rafRef.current !== null) cancelAnimationFrame(rafRef.current);
    let pos = 0;
    const STEP = Math.max(1, Math.ceil(content.length / 90)); // finish in ~90 frames (~1.5s)

    const tick = () => {
      if (abortRef.current?.signal.aborted) {
        setMessages((prev) =>
          prev.map((m) => (m.streaming ? { ...m, content, streaming: false } : m))
        );
        setStreaming(false);
        rafRef.current = null;
        return;
      ...
      // Target the streaming placeholder directly — more robust than relying on last index.
      setMessages((prev) =>
        prev.map((m) =>
          m.streaming ? { ...m, content: content.slice(0, pos), streaming: !isDone } : m
        )
      );
  ...
  const sendMessage = async (text?: string) => {
    ...
    // Cancel any pending typewriter animation
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/v1/ai/chat`, {
        ...
        body: JSON.stringify({ sessionId: sessionId.current, message: msg, keyword: msg }),
      ...
      // Step 2: Poll DB every 500ms until new assistant message appears (max 90s)
      await new Promise<void>((resolve, reject) => {
        ...
        const poll = async () => {
          ...
          if (polls++ >= MAX) { reject(new Error("Hết thời gian chờ. Vui lòng thử lại.")); return; }
          ...
          try {
            ...
            if (r.ok) {
              ...
              const data = (Array.isArray(raw) ? raw : raw?.data ?? raw?.result ?? []) as Array<{ role: string; content: string; ts: number }>;
              ...
              const newReply = data
                .filter((m) => m.role === "assistant" && m.ts > latestKnownAssistantTs)
                .sort((a, b) => a.ts - b.ts)
                .at(-1);
      ...
      const errMsg = err instanceof Error ? err.message : "Đã có lỗi. Vui lòng thử lại.";
      setMessages((prev) =>
        prev.map((m, i) =>
          i === prev.length - 1
        ...
        {open && (
          ...
          >
            ...
            >
              ...
              <Box
                sx={{
                  background: "linear-gradient(135deg, #f25c05 0%, #e64a19 100%)",
                  px: 2,
                  py: 1.5,
                  display: "flex",
                  alignItems: "center",
                ...
                <Stack direction="row" alignItems="center" spacing={1.5}>
                  ...
                  <Box>
                    ...
                    <Stack direction="row" alignItems="center" spacing={0.5} mt={0.2}>
                      ...
                      />
                      <Typography variant="caption" sx={{ color: "rgba(255,255,255,0.88)", fontSize: 11 }}>
                ...
                <Stack direction="row" spacing={0.5} alignItems="center">
                  <Tooltip title="Cuộc trò chuyện mới">
                    <IconButton size="small" onClick={handleClear} disabled={streaming}>
                      <DeleteOutlineIcon sx={{ color: "rgba(255,255,255,0.85)", fontSize: 19 }} />
                    </IconButton>
                  </Tooltip>
              ...
              {aiOffline && (
                ...
                >
                  <Box sx={{ width: 7, height: 7, borderRadius: "50%", bgcolor: "#ff9800", flexShrink: 0 }} />
              ...
              >
                  ...
                  <Stack spacing={1} sx={{ mt: 1 }}>
                    {[80, 60, 90].map((w, i) => (
                      <Box key={i} sx={{ display: "flex", alignItems: "flex-end", gap: 1 }}>
                        ...
                        <Skeleton variant="rounded" width={`${w}%`} height={36} sx={{ borderRadius: "4px 14px 14px 14px" }} />
                      </Box>
                    ))}
                  </Stack>
                )}
                    ...
                    {msg.role === "assistant" && (
                      <Box
                        sx={{
                          ...
                          height: 30,
                          borderRadius: "50%",
                          background: "linear-gradient(135deg,#f25c05,#e64a19)",
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                          flexShrink: 0,
                    ...
                    >
                      <Box
                        sx={{
                          ...
                          color: msg.role === "user" ? "#fff" : msg.isError ? "#c62828" : "#1a1a1a",
                      ...
                      >
                        {msg.content ? (
                          ...
                          <Box sx={{ display: "flex", gap: 0.5, alignItems: "center", py: 0.4 }}>
                            {[0, 1, 2].map((i) => (
                              <Box
                                key={i}
                                sx={{
                ...
                {!loadingHistory && showQuickReplies && (
                  <Box sx={{ mt: 0.5 }}>
                    <Typography variant="caption" color="text.secondary" sx={{ pl: 0.5, mb: 0.75, display: "block" }}>
                      Câu hỏi thường gặp:
                    </Typography>
                ...
                {/* ── Product links ── */}
                {!loadingHistory && isLastBotDone && productLinks.length > 0 && (
                  <Box sx={{ ml: 5, mt: 0.5 }}>
                      ...
                      {followUps.map((q) => (
                        <Chip
                          ...
                          sx={{
                            ...
                            color: "#555",
                            "&:hover": { bgcolor: "#f5f5f5", borderColor: "#f25c05", color: "#f25c05" },
                          }}
                        />
                      ))}
                    </Box>
                ...
                {!loadingHistory && isLastBotDone && needsEscalation && (
                  ...
                  >
                    <HeadsetMicIcon sx={{ color: "#f25c05", fontSize: 22, flexShrink: 0 }} />
                    <Box flex={1}>
