...
export default function NewsListView() {
  ...
  // gọi API (server-side filter bằng q); vẫn filter client khi dùng seed fallback
  const { data, isLoading } = useNewsList(keyword, 1, 12);
  const serverPosts = data?.result ?? [];
  const list = serverPosts.length ? serverPosts : seedPosts;

  const filteredPosts = useMemo(() => {
    if (serverPosts.length) return serverPosts; // đã lọc ở server
    const q = keyword.trim().toLowerCase();
    return list.filter((i) => i.title.toLowerCase().includes(q));
  }, [list, keyword, serverPosts.length]);
                    ...
                    >
                      ...
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {item.date || (item as any).publishedAt}
                        </Typography>
                      </Box>
                    </Box>
                  </motion.div>
