...
export default function CartItemList({
  ...
  const handleToggleAll = () => {
    ...
    onSelectionChange(allSelected ? new Set() : new Set(items.map(itemKey)));
  };
  ...
  const handleQuantityChange = async (item: CartItem, newQuantity: number) => {
    ...
    try {
      ...
      setUpdatingIds((prev) => { const n = new Set(prev); n.delete(key); return n; });
    }
  };
    ...
    <Box>
      ...
      >
        <Typography
          ...
          lineHeight={1.4}
          mb={0.5}
          sx={{ "&:hover": { color: "#f25c05", textDecoration: "underline" } }}
        >
          {item.productName}
        </Typography>
      </Link>
  ...
  const QtyControl = ({ item }: { item: CartItem }) => {
    ...
    return (
      <Box display="flex" alignItems="center" gap={0.5}>
        ...
        >
          {isUpdating ? <CircularProgress size={14} /> : item.quantity}
        </Box>
        <IconButton
          size="small"
          onClick={() => handleQuantityChange(item, item.quantity + 1)}
              ...
              return (
                <TableRow
                  ...
                  sx={{
                    ...
                    opacity: isDeleting ? 0.4 : selected ? 1 : 0.45,
                  }}
                >
                  {onSelectionChange && (
                    <TableCell padding="checkbox">
                      <Checkbox checked={selected} onChange={() => handleToggleItem(item)} size="small" />
                    </TableCell>
                  )}
                  <TableCell>
                    <Avatar
          ...
          {items.map((item) => {
            ...
            return (
              <MotionPaper
                ...
                animate={{ opacity: isDeleting ? 0.4 : selected ? 1 : 0.45, y: 0 }}
                transition={{ duration: 0.25 }}
              >
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  ...
                  <Box textAlign="right">
                    ...
                    <Typography fontWeight={700} fontSize={15} color="primary.main" mt={0.25}>
                      Tổng: {item.totalPrice.toLocaleString("vi-VN")}₫
                    </Typography>
                  </Box>
                </Box>
              </MotionPaper>
