...
export default function BrandAccordionSection({ brands }: Props) {
  ...
  // Only use active brands; fall back to all if none are active
  ...
  // All products flattened from categories
          ...
          <motion.div variants={itemVariants}>
            <Stack direction="row" spacing={1} sx={{ mb: 4, flexWrap: "wrap", gap: 1, justifyContent: "center" }}>
              {displayBrands.map((brand, index) => (
                <motion.div key={brand.id} whileHover={{ y: -2 }} whileTap={{ scale: 0.95 }}>
                  <Chip
                    avatar={<Avatar src={brand.logo} alt={brand.name}>{brand.name.charAt(0)}</Avatar>}
                    label={brand.name}
                    onClick={() => { setSelectedBrand(index); setTabValue(0); }}
                    sx={{
                      ...
                      px: 2,
                      py: 3,
                      "&:hover": { bgcolor: selectedBrand === index ? "#e64a19" : "action.hover" },
                    }}
                  />
                </motion.div>
              ))}
                ...
                <Grid size={{ xs: 12, md: 5 }}>
                  ...
                  >
                    ...
                    <Stack direction="row" spacing={3} alignItems="center" sx={{ mb: 3 }}>
                      <Box
                        sx={{
                          ...
                          boxShadow: 2, position: "relative", flexShrink: 0,
                        }}
                      >
                        <Image src={currentBrand.logo} alt={currentBrand.name} fill style={{ objectFit: "contain" }} />
                      </Box>
                      <Box>
                      ...
                      {currentBrand.website && (
                        <Stack direction="row" alignItems="center" spacing={1.5}>
                          ...
                          <Link
                            ...
                            target="_blank"
                            rel="noopener"
                            sx={{ color: "#f25c05", textDecoration: "none", fontSize: 14, "&:hover": { textDecoration: "underline" } }}
                          >
                    ...
                    {currentBrand.description && (
                      <Paper
                        elevation={0}
                        sx={{ p: 2, bgcolor: "action.hover", borderRadius: 3, border: "1px dashed #ffb700", mb: 3 }}
                      >
                        <Typography
                          variant="body2"
                          fontStyle="italic"
                  ...
                  >
                    ...
                    {tabValue === 0 && (
                      <Stack spacing={2}>
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.8 }}>
                          {currentBrand.description ||
                            `${currentBrand.name} là thương hiệu chính hãng được phân phối tại cửa hàng. Chúng tôi cung cấp ${currentBrand.productCount} sản phẩm chính hãng với đầy đủ bảo hành.`}
                        </Typography>
                    ...
                    {tabValue === 1 && (
                      <Stack spacing={1.5}>
                        ...
                        {currentBrand.category.length === 0 ? (
                          <Typography variant="body2" color="text.secondary">Chưa có danh mục sản phẩm</Typography>
                        ) : (
                          currentBrand.category.map((cat: CategoryRef) => (
                            <Paper
                              key={cat.id}
                              elevation={0}
                              onClick={() => router.push(`/product?brand=${currentBrand.slug}&category=${cat.slug}`)}
                              sx={{
                                p: 2, bgcolor: "action.hover", borderRadius: 3,
                    ...
                    {tabValue === 2 && (
                      <Stack spacing={1.5}>
                        ...
                        {allProducts.length === 0 ? (
                          <Typography variant="body2" color="text.secondary">Chưa có sản phẩm nào</Typography>
                        ) : (
                          <>
